...
def process_frame(frame, reference_face_encoding, frame_count, frame_rate, output_dir, saved_face_encodings):
    ...
    for i, (face_encoding, face_location) in enumerate(zip(face_encodings, face_locations)):
        ...
        if match[0]:
            ...
            if not already_saved:
                ...
                print(f"Match found at {timestamp_minutes} minutes {timestamp_seconds:.2f} seconds")
                matched = True

                # Save the matched face encoding
                ...
                face_filename = os.path.join(output_dir, f"match_{timestamp_minutes}m_{timestamp_seconds:.2f}s_face_{i}.jpg")
                cv2.imwrite(face_filename, matched_face)
...
while True:
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        future = executor.submit(process_frame, frame, reference_face_encoding, frame_count, frame_rate, output_dir, saved_face_encodings)
